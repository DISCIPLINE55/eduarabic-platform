...
export default function CertificatesPage() {
  ...
  const exportPDF = async (cert: any) => {
    ...
    try {
      ...
      doc.text((institution?.name || 'DisciNet Institution').toUpperCase(), W / 2, 28, { align: 'center' });
      ...
      const bodyText = cert.description ||
        `has successfully fulfilled all requirements for the ${certTypeTitle.toLowerCase()} at ${institution?.name || 'DisciNet'}.`;
      ...
      doc.text(`Verify at: discinet.app/verify/${cert.certificate_code}`, W - 20, H - 20, { align: 'right' });
