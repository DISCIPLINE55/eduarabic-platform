  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-users`, {
        ...
        body: JSON.stringify({ email, password, full_name, newRole: role, organization_id: organization_id || null }),
      });
      const result = await res.json();
      if (!res.ok) { toast.error(result.error || 'Failed to create user'); return; }
      toast.success(`${full_name || email} created as ${ROLE_LABELS[role]}. They can log in immediately.`);
      setShowCreate(false);
                ...
                {loading ? (
                  ...
                  <TableRow key={u.id} className={selected.has(u.id) ? 'bg-primary/5' : ''}>
                    ...
                    <TableCell className="whitespace-nowrap text-right">
                      {u.role === 'super_admin' ? (
                        <Badge variant="outline" className={ROLE_COLORS['super_admin'] + ' text-xs'}>Protected</Badge>
                      ) : (
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={editRole} onValueChange={v => setEditRole(v as UserRole)}>
                  ...
                  <SelectContent>
                    {ALL_ROLES.filter(r => r !== 'super_admin').map(r => <SelectItem key={r} value={r}>{ROLE_LABELS[r]}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
