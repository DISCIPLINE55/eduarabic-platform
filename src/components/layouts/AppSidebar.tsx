...
import {
  ...
  Link2, ClipboardCheck, Compass, Search, PlusCircle, Sparkles
} from 'lucide-react';
  ...
  switch (role) {
    case 'super_admin':
      return [
        ...
        { label: 'Invite Links', icon: Link2, path: '/super-admin/invite-links' },
        { label: 'Analytics', icon: BarChart3, path: '/super-admin/analytics' },
        { label: 'Audit Logs', icon: ClipboardCheck, path: '/super-admin/audit-logs' },
    ...
    case 'admin':
      return [
        ...
        { label: 'Enrollment Requests', icon: ClipboardCheck, path: '/admin/enrollments' },
    ...
    case 'teacher':
      return [
        ...
        { label: 'AI Hifz Tutor', icon: Sparkles, path: '/teacher/hifz-tutor' },
    ...
    case 'guest':
      return [
        { label: 'Welcome', icon: Compass, path: '/guest' },
        { label: 'Explore Institutions', icon: Search, path: '/guest/explore' },
        { label: 'Quran Viewer', icon: BookText, path: '/guest/quran' },
        { label: 'Enroll in Institution', icon: PlusCircle, path: '/guest/enroll' },
      ];
    case 'student':
    default:
      return [
        ...
        { label: 'AI Hifz Tutor', icon: Sparkles, path: '/student/hifz-tutor' },
...
function getRoleLabel(role: UserRole): string {
  const labels: Record<UserRole, string> = {
    ...
    teacher: 'Teacher', parent: 'Parent', student: 'Student', guest: 'Guest Explorer',
  };
  ...
  return (
    <div className="flex flex-col h-full bg-sidebar">
      ...
      <div className={cn('flex items-center h-16 px-3 border-b border-sidebar-border shrink-0', collapsed ? 'justify-center' : 'gap-2')}>
        {collapsed
          ? <DisciNetLogo size={32} variant="icon" theme="dark" className="shrink-0" />
          : <DisciNetLogo size={32} variant="full" theme="dark" className="flex-1 min-w-0" />
        }
