...
serve(async (req: Request): Promise<Response> => {
  ...
  if (method === "POST") {
    ...
    if (role === "super_admin") {
      ...
      const allowed = ["teacher", "student", "parent", "secretary"];
      if (!allowed.includes(newRole)) return json({ error: "Admins can only create teacher/student/parent/secretary accounts" }, 403);
    } else {
      return json({ error: "Forbidden" }, 403);
    }
    ...
    const { error: profileError } = await adminClient.from("profiles").upsert({
      id: newUser.user.id, email, full_name: full_name || null,
      role: newRole, organization_id: targetOrgId, is_profile_complete: true,
    });
  ...
  if (method === "PATCH") {
    ...
    const { data: targetProfile } = await adminClient
      .from("profiles").select("role, organization_id").eq("id", userId).maybeSingle();
    ...
    if (role === "super_admin") {
      ...
      const allowed = ["teacher", "student", "parent", "secretary"];
      if (!allowed.includes(newRole)) return json({ error: "Admins can only assign teacher/student/parent/secretary roles" }, 403);
